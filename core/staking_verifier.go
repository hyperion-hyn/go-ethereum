...
func checkDuplicateFields(
    ...
    validator common.Address, identity string, blsKeys []types.BLSPublicKey,
) error {
    ...
    blsKeyMap := map[types.BLSPublicKey]struct{}{}
...
func VerifyAndCreateValidatorFromMsg(
    ...
    // ATLAS(zgx): should ensure Amount larger than MinSelfDelegation
    ...
    if stateDB.IsValidator(msg.ValidatorAddress) {
        return nil, errors.Wrapf(
            errValidatorExist, common.MustAddressToBech32(msg.ValidatorAddress),
        )
    }
...
func VerifyAndCollectRewardsFromDelegation(
    ...
    for i := range delegations {
        ...
        if uint64(len(wrapper.Delegations)) > delegation.Index {
            ...
            log.Warn("Delegation index out of bound",
                "validator", delegation.ValidatorAddress.String(),
                "delegation index", delegation.Index,
                "delegations length", len(wrapper.Delegations))
