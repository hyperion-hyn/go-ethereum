...
func checkDuplicateFields(state vm.StateDB, validator common.Address, identity string, blsKeys []staking.BLSPublicKey) error {
	// TODO(storage): read validator list from statedb
	addrs, err := state.ReadValidatorList()
	if err != nil {
		return err
	}
	...
	blsKeyMap := map[staking.BLSPublicKey]struct{}{}
...
func VerifyAndCreateMap3NodeFromMsg(
	stateDB vm.StateDB, epoch *big.Int, blockNum *big.Int, msg *staking.CreateMap3Node,
