...
var (
	...
	testDelegate, zeroDelegate               Redelegate
	testUndelegate, zeroUndelegate           Unredelegate
	testCollectReward, zeroCollectReward     CollectRedelegationRewards
)
...
func TestDirective_String(t *testing.T) {
	tests := []struct {
		...
		{DirectiveDelegate, "Redelegate"},
		{DirectiveUndelegate, "Unredelegate"},
		{DirectiveCollectRewards, "CollectRedelegationRewards"},
		{0xff, "Directive 255"},
	}
...
func TestDelegate_Copy(t *testing.T) {
	tests := []struct {
		...
		{Redelegate{}}, // empty values
	}
	for i, test := range tests {
		cp := test.d.Copy().(Redelegate)
...
func assertDelegateDeepCopy(d1, d2 Redelegate) error {
	if !reflect.DeepEqual(d1, d2) {
		return fmt.Errorf("not deep equal")
	}
	if &d1.DelegatorAddress == &d2.DelegatorAddress {
		return fmt.Errorf("MicrodelegatorAddress same pointer")
	}
...
func TestUndelegate_Copy(t *testing.T) {
	tests := []struct {
		...
		{Unredelegate{}}, // empty values
	}
	for i, test := range tests {
		cp := test.u.Copy().(Unredelegate)
...
func assertUndelegateDeepCopy(u1, u2 Unredelegate) error {
...
func TestCollectRewards_Copy(t *testing.T) {
	tests := []struct {
		cr CollectRedelegationRewards
	}{
		{testCollectReward}, // non-zero values
		{zeroCollectReward}, // zero values
	}
	for i, test := range tests {
		cp := test.cr.Copy().(CollectRedelegationRewards)
...
func assertCollectRewardDeepEqual(cr1, cr2 CollectRedelegationRewards) error {
...
func assertPubsCopy(s1, s2 []BLSPublicKey) error {
	if len(s1) != len(s2) {
...
func assertSigsCopy(s1, s2 []BLSSignature) error {
	if len(s1) != len(s2) {
		return fmt.Errorf("size not equal")
	}
	for i := range s1 {
...
func cpTestDataSetup() {
	...
	var zeroBLSPub BLSPublicKey
	var zeroBLSSig BLSSignature
	...
	testCreateValidator = CreateValidator{
		...
		SlotPubKeys:        []BLSPublicKey{blsPubSigPairs[0].pub},
		SlotKeySigs:        []BLSSignature{blsPubSigPairs[0].sig},
		Amount:             twelveK,
	}
	zeroCreateValidator = CreateValidator{
		...
		SlotPubKeys:        make([]BLSPublicKey, 0),
		SlotKeySigs:        make([]BLSSignature, 0),
		Amount:             common.Big0,
	}
	...
	testDelegate = Redelegate{
		DelegatorAddress: common.BigToAddress(common.Big1),
		ValidatorAddress: validatorAddr,
		Amount:           twelveK,
	}

	testUndelegate = Unredelegate{
		DelegatorAddress: common.BigToAddress(common.Big1),
		ValidatorAddress: validatorAddr,
		Amount:           twelveK,
	}
	zeroUndelegate = Unredelegate{
		Amount: common.Big0,
	}

	testCollectReward = CollectRedelegationRewards{
		DelegatorAddress: common.BigToAddress(common.Big1),
	}
	zeroCollectReward = CollectRedelegationRewards{}
}
...
// 	if decodedMsg.MicrodelegatorAddress.String() != msgDelegate.MicrodelegatorAddress.String() {
// 		t.Error("MicrodelegatorAddress does not match")
// 	}
...
// 	if decodedMsg.MicrodelegatorAddress.String() != msgRedelegate.MicrodelegatorAddress.String() {
// 		t.Error("MicrodelegatorAddress does not match")
// 	}
...
// 	if decodedMsg.MicrodelegatorAddress.String() != msgUndelegate.MicrodelegatorAddress.String() {
// 		t.Error("MicrodelegatorAddress does not match")
// 	}
