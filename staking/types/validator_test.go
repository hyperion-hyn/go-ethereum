...
func TestValidator_SanityCheck(t *testing.T) {
	tests := []struct {
		...
		{
			func(v *Validator) { v.SlotPubKeys = []BLSPublicKey{blsPubSigPairs[0].pub, blsPubSigPairs[0].pub} },
...
func makeDeployAccountsFromBLSPubs(pubs []BLSPublicKey) []genesis.DeployAccount {
...
func makeBLSPubSigPair() blsPubSigPair {
	...
	var shardPub BLSPublicKey
	copy(shardPub[:], blsPub.Serialize())

	var shardSig BLSSignature
	copy(shardSig[:], sig.Serialize())

	return blsPubSigPair{shardPub, shardSig}
}

func getPubsFromPairs(pairs []blsPubSigPair, indexes []int) []BLSPublicKey {
	pubs := make([]BLSPublicKey, 0, len(indexes))
