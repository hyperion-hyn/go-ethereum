...
var (
	...
	validatorPrototype = staking.Validator{
		...
		SlotPubKeys:          []BLSPublicKey{BLSPublicKey{}},
...
func GetDefaultValidatorWithAddr(addr common.Address, pubs []BLSPublicKey) staking.Validator {
	v := CopyValidator(validatorPrototype)
	v.Address = addr
	if pubs != nil {
		v.SlotPubKeys = make([]BLSPublicKey, len(pubs))
...
func GetDefaultValidatorWrapperWithAddr(addr common.Address, pubs []BLSPublicKey) staking.ValidatorWrapper {
	w := CopyValidatorWrapper(vWrapperPrototype)
	w.Address = addr
	if pubs != nil {
		w.SlotPubKeys = make([]BLSPublicKey, len(pubs))
