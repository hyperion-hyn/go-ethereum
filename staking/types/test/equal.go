...
// CheckValidatorWrapperEqual checks the equality of types.ValidatorWrapper. If not equal, an
// error is returned. Note nil pointer is treated as zero in this compare function.
func CheckValidatorWrapperEqual(w1, w2 types.ValidatorWrapper) error {
...
// error is returned. Note nil pointer is treated as zero in this compare function.
func CheckValidatorEqual(v1, v2 types.Validator) error {
...
func checkValidatorWrapperEqual(w1, w2 types.ValidatorWrapper) error {
	...
	if err := checkDelegationsEqual(w1.Redelegations, w2.Redelegations); err != nil {
...
func checkDelegationsEqual(ds1, ds2 types.Redelegations) error {
	if len(ds1) != len(ds2) {
...
func checkDelegationEqual(d1, d2 types.Redelegation) error {
	if d1.DelegatorAddress != d2.DelegatorAddress {
		return fmt.Errorf(".MicrodelegatorAddress not equal: %x / %x",
...
func checkUndelegationsEqual(uds1, uds2 types.Undelegations) error {
...
func checkUndelegationEqual(ud1, ud2 types.Undelegation) error {
...
func checkPubKeysEqual(pubs1, pubs2 []types.BLSPublicKey) error {
...
func checkDescriptionEqual(d1, d2 types.Description) error {
	if d1.Name != d2.Name {
...
func checkCommissionEqual(c1, c2 types.Commission) error {
...
func checkCommissionRateEqual(cr1, cr2 types.CommissionRates) error {
