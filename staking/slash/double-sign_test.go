...
func TestRate(t *testing.T) {
	tests := []struct {
		...
		{
			votingPower: makeVotingPower(map[BLSPublicKey]numeric.Dec{
...
func makeEmptyRecordWithSecondSignerKey(pub BLSPublicKey) Record {
	var r Record
	r.Evidence.SecondVote.SignerPubKey = pub
	return r
}

func makeVotingPower(m map[BLSPublicKey]numeric.Dec) *votepower.Roster {
	r := &votepower.Roster{
		Voters: make(map[BLSPublicKey]*votepower.AccommodateHarmonyVote),
	}
	for pub, pct := range m {
...
func defaultTestValidator(pubKeys []BLSPublicKey) staking.Validator {
...
func (kp blsKeyPair) Pub() BLSPublicKey {
