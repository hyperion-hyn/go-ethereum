...
func newTestPreprepare(v *atlas.View) *atlas.Preprepare {
	return &atlas.Preprepare{
		View:     v,
...
func TestHandlePreprepare(t *testing.T) {
	...
	testCases := []struct {
		system          *testSystem
		expectedRequest atlas.Proposal
		expectedErr     error
		existingBlock   bool
	}{
		{
...
func TestHandlePreprepareWithLock(t *testing.T) {
	...
	testCases := []struct {
		system       *testSystem
		proposal     atlas.Proposal
		lockProposal atlas.Proposal
	}{
		{
			newSystem(),
			proposal,
	...
	for _, test := range testCases {
		...
		lockPreprepare := &atlas.Preprepare{
			View:     curView,
