...
func (c *core) handleEvents() {
	...
	for {
		...
		case event, ok := <-c.finalCommittedSub.Chan():
			...
			case atlas.FinalCommittedEvent:
...
func (c *core) handleMsg(payload []byte) error {
	...
	_, src := c.valSet.GetBySigner(msg.Signer)
	if src == nil {
...
func (c *core) handleCheckedMsg(msg *message, src atlas.Validator) error {
	...
	case msgExpect:
		return testBacklog(c.handleExpect(msg, src))
	case msgConfirm:
		return testBacklog(c.handleConfirm(msg, src))
	case msgCommit:
...
func (c *core) handleTimeoutMsg() {
	...
	if lastProposal != nil && lastProposal.Number().Cmp(c.current.Sequence()) >= 0 {
		c.logger.Debug("round change timeout, catch up latest sequence", "number", lastProposal.Number().Uint64())
		c.startNewRound(common.Big0)
	} else {
