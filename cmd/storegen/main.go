...
import (
	...
	"github.com/ethereum/go-ethereum/cmd/storegen/wrapper"
...
var (
	...
	storeFlag = cli.StringFlag{
		Name:  "storage-layout",
		Usage: "Path to the Ethereum contract storage-layout json to bind, - for STDIN",
	}
...
func abigen(c *cli.Context) error {
	...
	if c.GlobalString(storeFlag.Name) != "" {
		// Load up the ABI, optional bytecode and type name from the parameters
		var (
			store []byte
			err   error
		)
		input := c.GlobalString(storeFlag.Name)
		if input == "-" {
			store, err = ioutil.ReadAll(os.Stdin)
		} else {
			store, err = ioutil.ReadFile(input)
		}
		if err != nil {
	...
	code, err := wrapper.Store(types, stores, c.GlobalString(pkgFlag.Name), lang)
	if err != nil {
